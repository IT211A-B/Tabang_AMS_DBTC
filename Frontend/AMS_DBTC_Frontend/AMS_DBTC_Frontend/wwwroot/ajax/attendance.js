var Attendace Controller = {
    ...
    _renderRows: function (list) {
        ...
        $.each(list, function (i, s) {
            ...
            html += '<div class="trow tbl-grid" id="row-' + s.id + '">' +
                ...
                '<div><div class="s-nm">' + s.name + '</div><div class="s-id">' + s.id + '</div></div>' +
                '</div></div>' +
                ...
                '</div></div>' +
                '<div class="td"><input class="note-i" type="text" placeholder="Optional note…" ' +
    ...
    mark: function (id, status, btn) {
        ...
        if (STATE.attendance[id]) rec.remark = STATE.attendance[id].remark;
    ...
    setRemark: function (id, value) {
        if (STATE.attendance[id]) STATE.attendance[id].remark = value;
    },
    ...
    submit: function () {
        ...
        if (marked < STUDENTS.length) {
            if (!confirm('Only ' + marked + ' of ' + STUDENTS.length + ' students marked. Submit anyway?')) return;
        }
        Helpers.addActivity( 'Attendance submitted', $('#classSel').val() || '');
        Helpers.toast('Attendance submitted!', 'ok');
    }
};
